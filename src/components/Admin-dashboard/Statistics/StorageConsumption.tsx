    ...
    <ShadowedCard className="w-[30%] gap-0">
      <div className="flex justify-between items-center mb-2">
        <div className="flex gap-3 h-5">
          <Image
            src="/storage-consumption-icon.svg"
            width={20}
            height={20}
            alt="storage"
          />
          <p className="text-primary_font text-base font-medium leading-4">
            Storage Consumption
          </p>
        </div>
        ...
        <div className="w-[18rem] h-full">
          ...
          >
            <div style={{ width: '101%' }}>
              <CircularProgressbarWithChildren
                value={
                  ((storage?.alloted || 0) / (storage?.total || 100)) * 100
                }
                strokeWidth={5}
                circleRatio={0.6}
                styles={buildStyles({
      ...
      <div className="flex justify-between items-center z-50">
        ...
        <div className="flex gap-4 items-center justify-center">
          <p className="text-xs text-green-500 font-medium h-4">
            {(storage?.total || 0) - Math.ceil((storage?.used || 0) * 10) / 10}
            GB left
          </p>
