...
type Props = {
  fileName: string;
  setFileName: (_value: string) => void;
  selectedExtension: string;
  setSelectedExtension: (_value: string) => void;
};

const CreateFileSection = ({
  fileName,
  setFileName,
  selectedExtension,
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              ...
              className="p-2 px-6 flex gap-3 items-center text-primary_font rounded-sm bg-bg_hover"
            >
              {'.' + selectedExtension}
              <Image
                src="/down-arrow.svg"
            ...
            <DropdownMenuContent className="h-[15rem] overflow-auto">
              {fileExtensions.map((extension, index) => (
                <DropdownMenuItem
                  key={index}
                  title={extension}
                  onClick={() => handleSelectChange(extension)}
                >
                  {'.' + extension}
                </DropdownMenuItem>
              ))}
