...
type Props = {
  ...
  setRequestType: (_value: 'internal' | 'external') => void;
};
  ...
  return (
    <div className="flex flex-col h-[18rem] border-[1px] border-solid border-primary_font_2 rounded-lg">
      <div className="flex h-8 border-b-[1px] border-primary_font_2">
        <div
          className={`flex items-center justify-center w-[50%] cursor-pointer rounded-tl-md ${
            requestType === 'internal' && 'bg-primary_font_2'
          }`}
          onClick={() => setRequestType('internal')}
        >
          <p
            className={`${
              requestType === 'internal' ? 'text-white' : 'text-primary_font'
            } font-normal text-sm`}
          >
            Internal request
          </p>
        ...
        <div
          className={`flex items-center justify-center w-[50%] cursor-pointer rounded-tr-md ${
            requestType === 'external' && 'bg-primary_font_2'
          }`}
          onClick={() => setRequestType('external')}
        >
          <p
            className={`${
              requestType === 'external' ? 'text-white' : 'text-primary_font'
            } font-normal text-sm`}
          >
            External request
          </p>
