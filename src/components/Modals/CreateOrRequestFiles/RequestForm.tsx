...
const RequestForm = ({request, requests, setRequests}: Props) => {
  ...
  const handleInputChange = (id: string, field: keyof Request) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className='flex flex-col bg-bg_hover gap-3 p-2 py-3 rounded-md'>
      <div className='flex gap-1 flex-col w-full'>
        <div className='flex justify-between'>
          <div className='flex gap-2'>
            <Image src='/file-icon.svg' width={18} height={18} alt='Folder icon'/>
            <p className='text-primary_font font-normal text-sm'>Request file name</p>
          </div>
          {requests.length > 1 && (
              <button className='w-5 h-5 p-[0.3rem] rounded-full bg-[#F0F0F0] mt-0' onClick={() => handleRemoveRequest(request.id)}>
      ...
      <div className='flex gap-1 flex-col w-full'>
          <div className='flex gap-2'>
            <Image src='/user-icon.svg' width={18} height={18} alt='Folder icon'/>
            <p className='text-primary_font font-normal text-sm'>Request from user</p>
          </div>
        ...
        <input 
          ...
          className={`p-2 px-4 w-full text-secondary_font rounded-sm text-sm ${!request.isEmailValid && 'border-[1px] border-solid border-[#FF0000]'}`}
          required
        />
      </div>
    </div>
