    ...
    <div className="w-[45%] flex flex-col gap-3 bg-[#E5EDFF] rounded-xl px-4 p-3">
      ...
      <div>
        <div className="flex justify-between items-end">
          <p className="text-primary_font text-[0.8rem]">
            <span className="font-medium">
              {user?.storage.storage_used || 0}
            </span>
            /{user?.storage.total_storage || 0} GB
          </p>
          <p className="text-green-500 text-xs">
            {parseInt(user?.storage.total_storage || '1') -
              parseInt(user?.storage.storage_used || '1')}{' '}
            GB left
          </p>
        </div>
        <Progress
