export default async function groupShare(settings: InternalShareSettings, groupUrlHashes: string[], groupSelectedFile: any[], groupSelectedFolder: any[]) {
    const groupData = {
      group_hash: groupUrlHashes,
      file_hash: groupSelectedFile?.map((content) => content.urlhash),
      folder_hash: groupSelectedFolder?.map((content) => content.urlhash),
